...
(function (global) {
  ...
  function renderDaily(result) {
    ...
    const measure = cumulative ? "Cumulative Premium" : "Daily Premium";
    element("dailyMovementTable").innerHTML = `<table><thead><tr><th>Day</th><th>Base Month ${measure}</th><th>Comparison Month ${measure}</th><th>Change</th><th>Growth / Degrowth</th><th>Direction</th></tr></thead><tbody>${rows}</tbody></table>`;
  }
  ...
  const PACE_ALL_CHANNELS_VALUE = "__PACE_ALL_CHANNELS__";
  function syncPaceEntity(result) {
    const entities = result && result.entities || [];
    if (state.comparison.dimension === "BANK") {
      if (state.comparison.paceSelectionMode !== "ENTITY" || !entities.some((item) => item.key === state.comparison.paceEntityKey)) {
        state.comparison.paceSelectionMode = "ALL_CHANNELS";
        state.comparison.paceEntityKey = null;
      }
      return;
    }
    state.comparison.paceSelectionMode = "ENTITY";
  ...
  function selectedPaceView() {
    if (state.comparison.dimension === "BANK" && state.comparison.paceSelectionMode === "ALL_CHANNELS") {
      return { result: lastPaceOverallResult, entity: paceEntity(lastPaceOverallResult) };
    }
    return { result: lastPaceDimensionResult, entity: paceEntity(lastPaceDimensionResult) };
  }
  function renderPaceControls(result) {
    ...
    const entities = result && result.entities || [];
    const allChannels = state.comparison.dimension === "BANK" ? option(PACE_ALL_CHANNELS_VALUE, "All Channels", state.comparison.paceSelectionMode === "ALL_CHANNELS") : "";
    element("paceEntity").innerHTML = allChannels + entities.map((item) => option(item.key, item.label, state.comparison.paceSelectionMode === "ENTITY" && item.key === state.comparison.paceEntityKey)).join("");
  ...
  function renderPaceKpis(entity) {
    ...
    const values = summary ? [["Base Month Premium Through Day", money(summary.baseThroughDayActual), summary.baseThroughDayActual], ["Comparison Month Premium Through Day", money(summary.comparisonThroughDayActual), summary.comparisonThroughDayActual], ["Absolute Gap", signedMoney(summary.absoluteGap), summary.absoluteGap], ["Growth / Degrowth", summary.growthPct === null ? "N/A" : growth(summary.growthPct), summary.growthPct], ["Compared Through Day", `Day ${summary.throughDay}`, summary.throughDay]] : [];
  ...
  function renderPaceChart(entity, result) {
    ...
    element("paceChart").innerHTML = `<div class="commercial-pace-legend"><span class="commercial-pace-base">Base ${escape(periodLabel(result.basePeriod))}</span><span class="commercial-pace-comparison">Comparison ${escape(periodLabel(result.comparisonPeriod))}</span></div><svg viewBox="0 0 ${width} ${height}" role="img" aria-labelledby="paceChartTitle"><title id="paceChartTitle">Cumulative Premium through Day ${escape(result.effectiveThroughDay)} for ${escape(entity.label)}</title><line x1="${left}" y1="${zeroY}" x2="${width - right}" y2="${zeroY}" stroke="#94a3b8"/><path d="${pathFor("baseCumulativeActual")}" fill="none" stroke="#17375e" stroke-width="3"/><path d="${pathFor("comparisonCumulativeActual")}" fill="none" stroke="#795600" stroke-width="3"/><text x="${left}" y="${height - 10}" fill="#334155">Day 1</text><text x="${width - right - 40}" y="${height - 10}" fill="#334155">Day ${escape(result.effectiveThroughDay)}</text></svg>`;
  }
  function renderPaceTable(entity) {
    if (!entity || !entity.days || !entity.days.length) { element("paceTable").innerHTML = `<p class="empty-state">No equivalent elapsed-day detail is available.</p>`; return; }
    ...
    element("paceTable").innerHTML = `<table><thead><tr><th>Day</th><th>Base Month Daily Premium</th><th>Comparison Month Daily Premium</th><th>Daily Change</th><th>Base Month Cumulative Premium</th><th>Comparison Month Cumulative Premium</th><th>Cumulative Gap</th><th>Cumulative Growth</th></tr></thead><tbody>${rows}</tbody></table>`;
  }
  function renderPace(periodContext, performance, authorityContext) {
    ...
    const core = global.BancaTrackerCore;
    const scope = core && typeof core.getPerformanceContext === "function" ? core.getPerformanceContext() : null;
    const facts = scope && Array.isArray(scope.fullUploadData) ? scope.fullUploadData : core.state.factData || [];
    const common = { facts, performanceResult: performance, periodContext, basePeriod: state.comparison.basePeriod, comparisonPeriod: state.comparison.comparisonPeriod, authorityContext, throughDay: state.comparison.paceThroughDay };
    let result = global.BancaTrackerEquivalentElapsedDayComparison.buildComparison({ ...common, dimension: state.comparison.dimension });
    let overallResult = state.comparison.dimension === "BANK" ? global.BancaTrackerEquivalentElapsedDayComparison.buildComparison({ ...common, dimension: "OVERALL" }) : null;
    if (result.status === "INVALID_THROUGH_DAY" && state.comparison.paceThroughDay !== null) {
      state.comparison.paceThroughDay = null;
      result = global.BancaTrackerEquivalentElapsedDayComparison.buildComparison({ ...common, dimension: state.comparison.dimension, throughDay: null });
      overallResult = state.comparison.dimension === "BANK" ? global.BancaTrackerEquivalentElapsedDayComparison.buildComparison({ ...common, dimension: "OVERALL", throughDay: null }) : null;
    }
    ...
    lastPaceDimensionResult = result; lastPaceOverallResult = overallResult;
    syncPaceEntity(result); renderPaceControls(result);
    const selected = selectedPaceView(); const selectedResult = selected.result || result; const entity = selected.entity;
    renderPaceReadiness(selectedResult); renderPaceKpis(entity); renderPaceChart(entity, selectedResult); renderPaceTable(entity);
    ensurePaceExportButton(); if (element("paceExport")) element("paceExport").disabled = !entity || !entity.days.length || !["READY", "PARTIAL", "SAME_PERIOD"].includes(selectedResult.status);
    lastPaceResult = selectedResult;
  ...
  function updateMaturityMovementExport(result = lastMaturityResult, entity = selectedMaturityEntity(result)) { const transition = entity && entity.transitions && entity.transitions[state.maturity.transitionIndex]; const valid = result && ["READY", "PARTIAL"].includes(result.status); if (element("maturityMovementExport")) element("maturityMovementExport").disabled = !valid || !transition || !Array.isArray(transition.rows) || !transition.rows.length; }
  function clearMaturityExportState() { lastMaturityResult = null; if (element("maturityMovementExport")) element("maturityMovementExport").disabled = true; if (element("maturityDistributionExport")) element("maturityDistributionExport").disabled = true; }
  ...
  function renderMaturity(periodContext, performance, authorityContext) { if (!global.BancaTrackerBranchMaturityComparison) return null; syncMaturityPeriods(periodContext); const result = global.BancaTrackerBranchMaturityComparison.buildComparison({ performanceResult: performance, periodContext, selectedPeriods: state.maturity.periods, dimension: state.maturity.dimension, authorityContext }); lastMaturityResult = result; renderMaturityControls(periodContext, result); renderMaturityReadiness(result); const entity = selectedMaturityEntity(result); renderMaturityDistribution(entity); renderMaturityMovement(entity); const valid = entity && ["READY", "PARTIAL"].includes(result.status); if (element("maturityDistributionExport")) element("maturityDistributionExport").disabled = !valid || !entity.monthlyDistributions.length; updateMaturityMovementExport(result, entity); return result; }
  function exportScope(label, dimension) { return dimension === "OVERALL" ? "overall" : `${String(dimension || "entity").toLowerCase()}-${label || "entity"}`; }
  function downloadExport(rows, columns, datasetId, periods, scopeLabel) { if (!rows.length || !global.BancaTrackerCsvExport) return null; const csv = global.BancaTrackerCsvExport.serializeCsv({ rows, columns }); const filename = global.BancaTrackerCsvExport.buildFilename({ datasetId, periods, scopeLabel }); return global.BancaTrackerCsvExport.downloadCsv({ csv, filename }); }
  function exportPace() { const entity = paceEntity(lastPaceResult); if (!lastPaceResult || !entity || !["READY", "PARTIAL", "SAME_PERIOD"].includes(lastPaceResult.status) || !entity.days.length) return null; return downloadExport(entity.days, [{ key: "day", label: "Day" }, { key: "baseDailyActual", label: "Base Month Daily Premium" }, { key: "comparisonDailyActual", label: "Comparison Month Daily Premium" }, { key: "dailyAbsoluteChange", label: "Daily Absolute Change" }, { key: "dailyGrowthPct", label: "Daily Growth %" }, { key: "dailyDirection", label: "Daily Direction" }, { key: "baseCumulativeActual", label: "Base Month Cumulative Premium" }, { key: "comparisonCumulativeActual", label: "Comparison Month Cumulative Premium" }, { key: "cumulativeAbsoluteChange", label: "Cumulative Absolute Change" }, { key: "cumulativeGrowthPct", label: "Cumulative Growth %" }, { key: "cumulativeDirection", label: "Cumulative Direction" }], "equivalent-elapsed-day-comparison", [lastPaceResult.basePeriod, lastPaceResult.comparisonPeriod], exportScope(entity.label, lastPaceResult.dimension)); }
  function exportMaturityDistribution() { const entity = selectedMaturityEntity(lastMaturityResult); if (!lastMaturityResult || !entity || !["READY", "PARTIAL"].includes(lastMaturityResult.status)) return null; const rows = entity.monthlyDistributions.flatMap((distribution) => distribution.bands.map((band) => ({ period: distribution.periodKey, band: band.band, bandIndex: band.orderedBandIndex, branchCount: band.branchCount, populationPct: band.percentageOfPopulation, signedPremium: band.signedPremium, populationCount: distribution.populationCount }))); return downloadExport(rows, [{ key: "period", label: "Period" }, { key: "band", label: "Band" }, { key: "bandIndex", label: "Band Index" }, { key: "branchCount", label: "Branch Count" }, { key: "populationPct", label: "Population %" }, { key: "signedPremium", label: "Signed Premium" }, { key: "populationCount", label: "Population Count" }], "branch-maturity-distribution", lastMaturityResult.selectedPeriods || state.maturity.periods, exportScope(entity.label, lastMaturityResult.dimension)); }
  function exportMaturityMovement() { const entity = selectedMaturityEntity(lastMaturityResult); const transition = entity && entity.transitions && entity.transitions[state.maturity.transitionIndex]; if (!lastMaturityResult || !transition || !["READY", "PARTIAL"].includes(lastMaturityResult.status)) return null; return downloadExport(transition.rows, [{ key: "branchId", label: "Branch ID" }, { key: "branchName", label: "Branch Name" }, { key: "canonicalBank", label: "Bank" }, { key: "stateName", label: "State" }, { key: "zoneName", label: "Zone" }, { key: "baseMonth", label: "Base Month" }, { key: "basePremium", label: "Base Premium" }, { key: "baseBand", label: "Base Band" }, { key: "baseBandIndex", label: "Base Band Index" }, { key: "comparisonMonth", label: "Comparison Month" }, { key: "comparisonPremium", label: "Comparison Premium" }, { key: "comparisonBand", label: "Comparison Band" }, { key: "comparisonBandIndex", label: "Comparison Band Index" }, { key: "movement", label: "Movement" }, { key: "specialMovement", label: "Special Movement" }, { key: "presenceStatus", label: "Presence Status" }], "branch-maturity-movement", [transition.baseMonth, transition.comparisonMonth], exportScope(entity.label, lastMaturityResult.dimension)); }
  ...
  function handleComparisonDimensionChange(value) { state.comparison.dimension = value || element("comparisonDimension").value; state.comparison.selectedEntityKey = null; state.comparison.paceSelectionMode = state.comparison.dimension === "BANK" ? "ALL_CHANNELS" : "ENTITY"; state.comparison.paceEntityKey = null; return render(); }
  ...
  function handlePaceEntityChange(value) { const selectedValue = value || element("paceEntity").value; if (state.comparison.dimension === "BANK" && selectedValue === PACE_ALL_CHANNELS_VALUE) { state.comparison.paceSelectionMode = "ALL_CHANNELS"; state.comparison.paceEntityKey = null; } else { state.comparison.paceSelectionMode = "ENTITY"; state.comparison.paceEntityKey = selectedValue || null; } const selected = selectedPaceView(); lastPaceResult = selected.result; renderPaceReadiness(selected.result); renderPaceKpis(selected.entity); renderPaceChart(selected.entity, selected.result || {}); renderPaceTable(selected.entity); return selected.entity; }
  ...
  function handleMaturityEntityChange(value) { state.maturity.entityKey = value || element("maturityEntity").value || null; const entity = selectedMaturityEntity(lastMaturityResult); renderMaturityDistribution(entity); renderMaturityMovement(entity); updateMaturityMovementExport(lastMaturityResult, entity); return entity; }
  function handleMaturityTransitionChange(value) { state.maturity.transitionIndex = Number(value === undefined ? element("maturityTransition").value : value); const entity = selectedMaturityEntity(lastMaturityResult); renderMaturityMovement(entity); updateMaturityMovementExport(lastMaturityResult, entity); }
  ...
  function init() {
    ...
    if (element("paceExport")) element("paceExport").addEventListener("click", exportPace);
    ...
    if (element("maturityDistributionExport")) element("maturityDistributionExport").addEventListener("click", exportMaturityDistribution); if (element("maturityMovementExport")) element("maturityMovementExport").addEventListener("click", exportMaturityMovement);
