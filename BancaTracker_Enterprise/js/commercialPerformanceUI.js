...
(function (global) {
  ...
  function exportPace() { const entity = paceEntity(lastPaceResult); if (!lastPaceResult || !entity || !["READY", "PARTIAL", "SAME_PERIOD"].includes(lastPaceResult.status) || !entity.days.length) return null; return downloadExport(entity.days, [{ key: "day", label: "Day" }, { key: "baseDailyActual", label: "Base Daily Actual" }, { key: "comparisonDailyActual", label: "Comparison Daily Actual" }, { key: "dailyAbsoluteChange", label: "Daily Absolute Change" }, { key: "dailyGrowthPct", label: "Daily Growth %" }, { key: "dailyDirection", label: "Daily Direction" }, { key: "baseCumulativeActual", label: "Base Cumulative Actual" }, { key: "comparisonCumulativeActual", label: "Comparison Cumulative Actual" }, { key: "cumulativeAbsoluteChange", label: "Cumulative Absolute Change" }, { key: "cumulativeGrowthPct", label: "Cumulative Growth %" }, { key: "cumulativeDirection", label: "Cumulative Direction" }], "equivalent-elapsed-day-comparison", [lastPaceResult.basePeriod, lastPaceResult.comparisonPeriod], exportScope(entity.label, lastPaceResult.dimension)); }
