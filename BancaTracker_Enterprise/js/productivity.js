...
(function (global) {
  ...
  function entityBase(key, bank, code) { return { key, bank, code, premium: 0, records: 0, branchKeys: new Set(), names: new Set(), lobs: new Set(), productCodes: new Set(), baCodes: new Set() }; }
  function branchStatus(branch) { return { active: branch.premium >= config.THRESHOLDS.ACTIVE_BRANCH, near: branch.premium >= config.THRESHOLDS.NEAR_ACTIVE_MIN && branch.premium < config.THRESHOLDS.ACTIVE_BRANCH }; }
  function singleOr(values, blankLabel) { const list = [...values]; return list.length === 0 ? blankLabel : list.length === 1 ? list[0] : "Multiple mappings"; }
  function finalizeEntity(entity, derived, kind, audit) {
    let activeBranches = 0; let nearActiveBranches = 0; let aggregateActivationGap = 0;
    entity.branchKeys.forEach((key) => { const branch = derived.branchesByKey[key]; if (!branch) return; const status = branchStatus(branch); if (status.active) activeBranches += 1; if (status.near) { nearActiveBranches += 1; aggregateActivationGap += config.THRESHOLDS.ACTIVE_BRANCH - branch.premium; } });
    const observedBranches = entity.branchKeys.size;
    const mappingConflict = kind === "rm" && (entity.names.size > 1 || audit.baCodeConflicts.some((item) => item.key === entity.code));
    const premiumPerObservedBranch = observedBranches ? entity.premium / observedBranches : null;
    const premiumPerActiveBranch = activeBranches ? entity.premium / activeBranches : null;
    let cue = "";
    if (nearActiveBranches > 0) cue = kind === "imd" ? "High activation-conversion opportunity." : "Prioritize near-active branch conversion.";
    else if (kind === "rm" && entity.premium > 0 && activeBranches > 0 && activeBranches < observedBranches) cue = "Premium concentrated in fewer active branches.";
    return { ...entity, name: mappingConflict ? "Mapping conflict" : singleOr(entity.names, "Not mapped"), mappingConflict, observedBranches, activeBranches, nearActiveBranches, aggregateActivationGap, premiumPerObservedBranch, premiumPerActiveBranch, lobBreadth: entity.lobs.size, productBreadth: entity.productCodes.size, observedBaCodes: entity.baCodes.size, cue };
  }
  function concentration(opportunities, dimension, label) {
    const map = new Map();
    opportunities.forEach((branch) => {
      const name = dimension(branch);
      if (!map.has(name)) map.set(name, { dimension: label, name, nearActiveBranches: 0, aggregateGap: 0 });
      const item = map.get(name); item.nearActiveBranches += 1; item.aggregateGap += config.THRESHOLDS.ACTIVE_BRANCH - branch.premium;
    });
  ...
  function build(context, derived, audit) {
    ...
    const imdMetrics = [...imds.values()].map((item) => finalizeEntity(item, derived, "imd", quality)).sort((a, b) => b.premium - a.premium || a.code.localeCompare(b.code));
    const hierarchyConflictKeys = new Set(quality.hierarchyConflicts.map((item) => item.branchKey));
    const productConflictCodes = new Set(quality.productConflicts.map((item) => item.key));
    const unknownBanks = new Set(quality.bankQuality.unknownBanks);
    const branchMetrics = derived.branches.map((branch) => {
      const status = branchStatus(branch); const gap = Math.max(0, config.THRESHOLDS.ACTIVE_BRANCH - branch.premium);
    ...
    const concentrations = {
      bank: concentration(opportunities, (branch) => branch.bank, "Bank"), zone: concentration(opportunities, (branch) => branch.zoneLabel, "Zone"), state: concentration(opportunities, (branch) => branch.stateLabel, "State"), rm: concentration(opportunities, ownerRm, "BA Code / RM"), imd: concentration(opportunities, ownerImd, "Bank / IMD")
    };
    const productCompleteness = quality.completeness ? quality.completeness.find((item) => item.field === "productCode") : null;
    const ytdPremiumByBank = context.ytdPremiumByBank;
    const bankIndexes = {};
  ...
  function exportOpportunities() {
    const rows = latestResult && latestResult.opportunities;
    const exporter = global.BancaTrackerCsvExport;
    if (!rows || !rows.length || !exporter) return null;
    const columns = [{ key: "branch", label: "Branch" }, { key: "bank", label: "Bank" }, { key: "zoneLabel", label: "Zone" }, { key: "stateLabel", label: "State" }, { key: "baCodeLabel", label: "BA Code" }, { key: "rmLabel", label: "RM Name" }, { key: "imdLabel", label: "IMD" }, { key: "premium", label: "Current Premium" }, { key: "gap", label: "Gap to ₹25K" }, { key: "cue", label: "Cue" }];
