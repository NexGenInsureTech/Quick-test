...
(function (global) {
  ...
  function render(audit) {
    ...
    document.getElementById("premiumQuality").innerHTML = `<table><thead><tr><th>Premium Sign</th><th>Rows</th><th>Aggregate Premium</th></tr></thead><tbody><tr><td>Positive</td><td>${audit.premium.positiveRows}</td><td>${utils.formatRupees(audit.premium.positiveTotal)}</td></tr><tr><td>Zero</td><td>${audit.premium.zeroRows}</td><td>${utils.formatRupees(0)}</td></tr><tr><td>Negative ${severity("INFO")}</td><td>${audit.premium.negativeRows}</td><td>${utils.formatRupees(audit.premium.negativeTotal)}</td></tr></tbody></table><p class='scorecard-note'>Negative premium is preserved and may represent cancellation, refund, or adjustment; treatment requires a future business rule.</p>`;
