...
(function (global) {
  ...
  function build(data, config, importSummary) {
    ...
    const findings = [
      ...
      ...(premium.negativeRows ? [{ severity: "INFO", category: "Premium", message: `${premium.negativeRows} negative premium row(s) total ${utils.formatRupees(premium.negativeTotal)}.` }] : []),
  ...
  const guidanceFor = (type) => {
    const authority = global.BancaTrackerDataQualityGuidance;
    return authority ? authority.render(authority.lookupLegacy(type), html) : "";
  };
  function conflictTable(items, kind, limit, typeForItem) {
  ...
  function render(audit) {
    ...
    const legacyTypes = global.BancaTrackerDataQualityGuidance && global.BancaTrackerDataQualityGuidance.LEGACY_TYPES;
    document.getElementById("hierarchyConflicts").innerHTML = conflictTable(audit.hierarchyConflicts, "branch hierarchy", configRef.RENDER_LIMITS.DATA_QUALITY_HIERARCHY, () => legacyTypes && legacyTypes.HIERARCHY_CONFLICT);
    document.getElementById("identityConflicts").innerHTML = conflictTable(audit.identityConflicts, "identity", configRef.RENDER_LIMITS.DATA_QUALITY_IDENTITY, (item) => legacyTypes && (item.keyLabel === "BA Code" ? legacyTypes.BA_TO_RM_CONFLICT : item.keyLabel === "RM Name" ? legacyTypes.RM_TO_BA_CONFLICT : legacyTypes.PRODUCT_CONFLICT));
    ...
    document.getElementById("monthBankCoverage").innerHTML = `<div class='quality-grid'><div><strong>Fiscal months present</strong><p>${coverage(mq.configuredMonthsPresent)}</p><strong>Fiscal months absent</strong><p>${coverage(mq.configuredMonthsAbsent)}</p>${mq.configuredMonthsAbsent.length ? guidanceFor(legacyTypes && legacyTypes.CONFIGURED_MONTH_ABSENT) : ""}<strong>Unconfigured labels ${severity("WARNING")}</strong><p>${coverage(mq.unconfiguredMonths)}</p>${mq.unconfiguredMonths.length ? guidanceFor(legacyTypes && legacyTypes.UNCONFIGURED_MONTH) : ""}<strong>Accepted blank Month rows</strong><p>${mq.blankMonthRows}</p>${mq.blankMonthRows ? guidanceFor(legacyTypes && legacyTypes.BLANK_MONTH) : ""}</div><div><strong>Configured banks represented</strong><p>${coverage(bq.representedConfiguredBanks)}</p><strong>Configured banks without rows ${severity("INFO")}</strong><p>${coverage(bq.configuredBanksWithoutRows)}</p>${bq.configuredBanksWithoutRows.length ? guidanceFor(legacyTypes && legacyTypes.CONFIGURED_BANK_ABSENT) : ""}<strong>Unknown banks ${severity("WARNING")}</strong><p>${coverage(bq.unknownBanks)}</p>${bq.unknownBanks.length ? guidanceFor(legacyTypes && legacyTypes.UNKNOWN_BANK) : ""}</div></div>`;
    document.getElementById("premiumQuality").innerHTML = `<table><thead><tr><th>Premium Sign</th><th>Rows</th><th>Aggregate Premium</th></tr></thead><tbody><tr><td>Positive</td><td>${audit.premium.positiveRows}</td><td>${utils.formatRupees(audit.premium.positiveTotal)}</td></tr><tr><td>Zero</td><td>${audit.premium.zeroRows}</td><td>${utils.formatRupees(0)}</td></tr><tr><td>Negative ${severity("INFO")}${audit.premium.negativeRows ? guidanceFor(legacyTypes && legacyTypes.NEGATIVE_PREMIUM) : ""}</td><td>${audit.premium.negativeRows}</td><td>${utils.formatRupees(audit.premium.negativeTotal)}</td></tr></tbody></table><p class='scorecard-note'>Negative premium is preserved and may represent cancellation, refund, or adjustment; treatment requires a future business rule.</p>`;
