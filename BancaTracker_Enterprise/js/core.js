...
(function (global) {
  ...
  function commitImport(result, authorityContext) {
    ...
    const context = authorityContext || (geographyAuthority && geographyAuthority.getCachedContext()) || (assignmentAuthority && assignmentAuthority.getCachedContext()) || (branchAuthority && branchAuthority.getCachedContext());
    ...
    setStatus("Building analytics...", false); state.factData = geographyAuthority
      ? geographyAuthority.applyRecords(assignmentRows, context)
      : assignmentRows; result.rows = state.factData; state.headerMap = result.headerMap; state.filters.month = "ALL"; state.filters.bank = "ALL";
